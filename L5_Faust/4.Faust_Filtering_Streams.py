...
async def clickevent(clickevents):
    #
    # DONE: Filter clickevents to only those with a number higher than or
    #       equal to 100
    ...
    async for event in clickevents.filter(lambda x: x.number>=100):
        #
        # DONE: Send the message to the `popular_uris_topic` with a key and value.
        #
        await popular_uris_topic.send(key=event.email, value=event)
